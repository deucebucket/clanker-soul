...
def test_no_mood_yet_returns_unrestricted() -> None:
    ...
    assert assess_capability(_snap(mood=None), GovernorConfig()) == CapabilityLevel.UNRESTRICTED


def test_healthy_mood_returns_unrestricted() -> None:
    healthy = [145, 110, 160, 80, 130, 175, 135]
    assert assess_capability(_snap(mood=healthy), GovernorConfig()) == CapabilityLevel.UNRESTRICTED


def test_low_w_drops_to_non_destructive() -> None:
    """W=70 < default level1_w_floor=80 → level 1."""
    mood = [145, 110, 160, 80, 130, 70, 135]
    assert assess_capability(_snap(mood=mood), GovernorConfig()) == CapabilityLevel.NON_DESTRUCTIVE
...
def test_far_from_soul_drops_to_non_destructive() -> None:
    ...
    assert assess_capability(snap, GovernorConfig()) == CapabilityLevel.NON_DESTRUCTIVE


def test_w_below_50_drops_to_read_only() -> None:
    mood = [145, 110, 160, 80, 130, 40, 135]
    assert assess_capability(_snap(mood=mood), GovernorConfig()) == CapabilityLevel.READ_ONLY
...
def test_high_trauma_drops_to_read_only() -> None:
    ...
    assert assess_capability(snap, GovernorConfig()) == CapabilityLevel.READ_ONLY


def test_w_below_30_drops_to_voice_only() -> None:
    mood = [145, 110, 160, 80, 130, 25, 135]
    assert assess_capability(_snap(mood=mood), GovernorConfig()) == CapabilityLevel.VOICE_ONLY
...
def test_crisis_lockout_engages_when_enabled() -> None:
    ...
    mood = [10, 110, 160, 80, 130, 10, 135]
    assert assess_capability(_snap(mood=mood), cfg) == CapabilityLevel.CRISIS_LOCKOUT
...
def test_recovery_eases_restrictions_automatically() -> None:
    ...
    assert assess_capability(_snap(mood=bad_mood), cfg) == CapabilityLevel.READ_ONLY
    assert assess_capability(_snap(mood=recovered_mood), cfg) == CapabilityLevel.UNRESTRICTED
...
def test_custom_thresholds_override_defaults() -> None:
    ...
    assert assess_capability(_snap(mood=healthy_default), strict) == CapabilityLevel.NON_DESTRUCTIVE
