...
def _event(
    weight: float, source: str, direction: str = "EXTERNAL_REPORT", patterns=("ATMOSPHERIC_GRIEF",)
) -> IngestRecord:
    raw = Score(v=40, w=40, patterns=patterns, direction=direction, source=source)
    return IngestRecord(
        ts=0.0,
        agent_id="x",
        raw=raw,
...
def test_crisis_diagnosis_emergency_is_called_out() -> None:
    crisis = CrisisDiagnosis(
        is_emergency=True,
        summary="3 external-report events",
        confidence=0.9,
...
def test_crisis_diagnosis_spike_is_framed_differently() -> None:
    crisis = CrisisDiagnosis(
        is_emergency=False,
        summary="emotional spike from 5 directed events",
        confidence=0.8,
