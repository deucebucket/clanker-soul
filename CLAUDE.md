...
**`clanker_soul/plugins/`** — dependency-free host plugin helpers: `manifest.py` (`PluginManifest`, `parse_manifest_json`, standard plugin kinds), `master.py` (`parse_plugins_toml`, enablement/priority/settings overlay), `loader.py` (`PluginLoader`, compatibility/dependency checks, topological load order, entrypoint import). This is a library facility for hosts that want drag-drop runtime extensions; it is not a service manager and does not auto-wire into `SoulPlugin`.
...
- `docs/plugins.md` — standard plugin manifest and loader shape for hosts that want drag-drop extensions.
