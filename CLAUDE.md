...
**`clanker_soul/ui/`** — package gated behind the `[ui]` optional dependency group: `app.py` (`create_app(db_path) -> FastAPI` factory — testable without uvicorn), `__init__.py` (`launch(db_path, *, agent_id, port, host, log_level)` — blocking uvicorn launcher), `templates/{base,index}.html` (Jinja2; Tailwind + HTMX via CDN — no Node toolchain), `static/`. The CLI's `ui` subcommand uses `try: from clanker_soul.ui import launch` so this module's existence is the install-detector. Tests use `pytest.importorskip("fastapi")` to skip cleanly without the extra. Subsequent route work (#26-#29) extends `app.py` and adds templates.
